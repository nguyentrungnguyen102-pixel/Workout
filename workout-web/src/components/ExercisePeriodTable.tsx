...
export default function ExercisePeriodTable({ periodLogs, prevPeriodLogs, presets, periodDays, prevPeriodDays }: ExercisePeriodTableProps) {
  ...
  // periodLogs can be a partial "period to date" window (e.g. 3 days into
  // the current week) while prevPeriodLogs is a full closed prior period
  // (or, when both periods are already closed, the same length) — prorate
  // the previous total to the same day count so the "▼ -80%" delta isn't
  // just an artifact of comparing 3 days against 7.

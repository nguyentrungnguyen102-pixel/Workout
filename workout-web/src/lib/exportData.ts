...
const LOG_CSV_HEADERS = [
  ...
  'Tạ (kg)', 'Quãng đường (km)', 'Cường độ', 'Phút', 'Calo', 'Địa điểm', 'Ghi chú',
];
...
export interface BackupPayload {
  exportedAt: string;
  appVersion: string;
  logs: Array<Pick<WorkoutLog, 'id' | 'date' | 'exercises' | 'totalDurationMinutes' | 'intensity' | 'intensityScore' | 'caloriesEstimate' | 'notes' | 'location' | 'source'>>;
...
export function buildBackupPayload(logs: WorkoutLog[], bodyMetrics: BodyMetric[]): BackupPayload {
  return {
    ...
    logs: [...logs]
      ...
      .map(({ id, date, exercises, totalDurationMinutes, intensity, intensityScore, caloriesEstimate, notes, location, source }) => ({
        id, date, exercises, totalDurationMinutes, intensity, intensityScore, caloriesEstimate, notes, location, source,
      })),

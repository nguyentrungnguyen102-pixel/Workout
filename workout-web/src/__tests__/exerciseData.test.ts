...
import { MET_TABLE, CATEGORY_MET } from '../lib/energy';
...
describe('SYSTEM_PRESETS data completeness', () => {
  ...
  it('every preset either has its own MET value or falls back to a category MET', () => {
    const missing = SYSTEM_PRESETS.filter((p) => !MET_TABLE[p.id] && !CATEGORY_MET[p.category]).map((p) => p.id);
    expect(missing).toEqual([]);
  });
});

...
describe('logsToCSV', () => {
  ...
  it('includes the location column when a log has one', () => {
    const csv = logsToCSV([makeLog('l1', '2026-07-10', { location: 'Sân bóng Q7' })]);
    expect(csv.split('\n')[0]).toContain('Địa điểm');
    expect(csv.split('\n')[1]).toContain('Sân bóng Q7');
  });

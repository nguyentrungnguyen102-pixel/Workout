...
// nhiều lần). Nguyên nhân chính: firebase-admin (Firestore qua gRPC) giữ
// kết nối nền mở nên process KHÔNG tự thoát dù main() đã chạy xong toàn bộ
// logic — phải process.exit() tường minh (xem cuối file). Bên cạnh đó,
// mọi lệnh gọi mạng (Firestore, Telegram) cũng được bọc timeout để job
// không treo giữa chừng nếu một request cụ thể bị mất mạng/không phản hồi.

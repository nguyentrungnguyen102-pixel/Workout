...
// Job GitHub Actions từng bị treo/cancelled vô thời hạn (quan sát thực tế
// nhiều lần, xem các PR #103/#111/#112/#113/#115/#117). Nguyên nhân gốc:
// firebase-admin (Firestore qua gRPC) giữ kết nối nền mở nên process KHÔNG
// tự thoát dù main() đã chạy xong toàn bộ logic — phải process.exit()
// tường minh (xem cuối file). Bên cạnh đó mọi lệnh gọi mạng (Firestore,
// Telegram) cũng được bọc timeout để job không treo giữa chừng nếu một
// request cụ thể bị mất mạng/không phản hồi.
...
// firebase-admin (Firestore qua gRPC) giữ kết nối nền mở nên Node KHÔNG tự
// thoát process dù main() đã resolve xong toàn bộ logic — nếu không có
// process.exit() tường minh ở đây, job GitHub Actions treo tới khi bị hệ
// thống tự huỷ (quan sát thực tế: "cancelled" không rõ lý do sau nhiều
// phút), chiếm slot và làm trễ mọi lần chạy cron /30 phút kế tiếp. Thoát
// tường minh ở cả 2 nhánh (thành công/lỗi) để job luôn kết thúc ngay khi
// logic xong, bất kể có handle nền nào còn mở.
main()
  .then(() => process.exit(0))
  .catch((err) => {

...
// v2.26.1 hotfix — none of the network calls below (Telegram fetch, Firestore
// reads/writes) previously had a timeout, and the GitHub Actions job itself
// had no timeout-minutes either. A single stalled connection (Telegram API
// or Firestore having a bad moment) could hang the job for GitHub's default
// 360-minute job ceiling, burning Actions minutes every time the */30 cron
// tick landed during the outage. These caps + the workflow's timeout-minutes
// (see telegram-reminder.yml) bound the whole run to a few minutes no matter
...
// of hanging forever; the underlying call itself is not cancelled (Firestore
// SDK calls aren't abortable), but the process moves on and reports it.
...
async function main() {
  ...
  // (e.g. firebase-admin's own connection setup, which happens outside any
  // withTimeout() call). Cleared on normal completion so it never delays a
  // healthy run.
